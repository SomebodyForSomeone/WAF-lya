...
1. **Context Middleware:** Проверяет сессию на аномалии (например, доступ к `/api/user/1`, затем `/api/user/2`...). Идентификатор ресурса извлекается по правилу из конфига. Если порог уникальных ресурсов превышен — IP блокируется.
